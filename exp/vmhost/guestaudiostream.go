...
// io.EOF when the source reaches its end. It returns only whole samples. It returns [io.ErrShortBuffer]
// for a non-empty b shorter than one sample (8 bytes) unless the source has ended. io.EOF does not close
// the stream: if the guest seeks the source back and plays again, a later Read yields its samples. Once
// the guest closes its player (or the session closes) Read reports io.EOF for good.
