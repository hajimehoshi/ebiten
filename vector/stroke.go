...
func addJoint(strokePath *Path, subPath *subPath, opIndex int, reverse bool, options *StrokeOptions) {
	...
	if !reverse {
		...
		dir0 = subPath.endDir(opIndex)
		dir1 = subPath.startDir(nextOpIdx)
	} else {
		nextOpIdx := opIndex - 1
		if nextOpIdx == -1 {
			if !subPath.closed {
		...
		dir0 = subPath.startDir(opIndex).inv()
		dir1 = subPath.endDir(nextOpIdx).inv()
	}
	...
	// TODO: Implement line joints.
	_ = dir0
	v := dir1.perp().norm().mul(options.Width / 2)
	p = p.add(v)
