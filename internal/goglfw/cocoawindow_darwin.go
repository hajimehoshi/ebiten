...
func (d *windowDelegate) WindowDidBecomeKey(cmd objc.SEL, _ objc.ID) {
	if _glfw.platformWindow.disableCursorWindow == d.window {
...
func (w *Window) createNativeWindow(wndconfig *wndconfig, fbconfig *fbconfig) error {
	w.platform.delegate = cocoa.NSObject_alloc(class_GLFWWindowDelegate).Send(sel_initWithGlfwWindow, unsafe.Pointer(w))
	...
	w.platform.object = cocoa.NSWindow{ID: cocoa.NSObject_alloc(class_GLFWWindow)}.InitWithContentRectStyleMaskBackingDefer(contentRect, styleMask, cocoa.NSBackingStoreBuffered, false).ID

	if w.platform.object == 0 {
		return fmt.Errorf("cocoa: failed to create window")
	}
	nsWindow := cocoa.NSWindow{ID: w.platform.object}
	...
	//
	w.platform.view = cocoa.NSObject_alloc(class_GLFWContentView).Send(sel_initWithGlfwWindow, w)
	...
	nsWindow.SetContentView(w.platform.view)
	nsWindow.SetDelegate(w.platform.delegate)
...
func (w *Window) platformDestroyWindow() error {
	...
	//
	win := cocoa.NSWindow{ID: w.platform.object}
	win.SetDelegate(0)
	//    [window->ns.delegate release];
	w.platform.delegate = 0

	view := cocoa.NSView{ID: w.platform.view}
	_ = view
...
func (w *Window) platformSetWindowTitle(title string) error {
	...
	win := cocoa.NSWindow{w.platform.object}
	win.SetTitle(t)
...
func (w *Window) platformShowWindow() {
	pool := cocoa.NSAutoreleasePool_new()
	defer pool.Release()
	cocoa.NSWindow{ID: w.platform.object}.OrderFront(0)
}
...
func (w *Window) platformWindowIconified() bool {
	pool := cocoa.NSAutoreleasePool_new()
	defer pool.Release()
	return cocoa.NSWindow{ID: w.platform.object}.IsMiniaturized()
}
...
func (w *Window) platformFocusWindow() error {
	...
	cocoa.NSWindow{ID: w.platform.object}.MakeKeyAndOrderFront(0)
	return nil
}
...
func (w *Window) platformGetWindowPos() (xpos, ypos int, err error) {
	...
	frame := cocoa.NSWindow{w.platform.object}.Frame()
	inv.SetArgumentAtIndex(unsafe.Pointer(&frame), 2)
	inv.InvokeWithTarget(w.platform.object)
	var contentRect cocoa.NSRect
...
func (w *Window) platformGetWindowSize() (width, height int, err error) {
	pool := cocoa.NSAutoreleasePool_new()
	defer pool.Release()
	contentRect := cocoa.NSView{ID: w.platform.view}.Frame()
...
func (w *Window) platformGetCursorPos() (xpos, ypos float64, err error) {
	...
	contentRect := cocoa.NSView{ID: w.platform.view}.Frame()
	// NOTE: The returned location uses base 0,1 not 0,0
	pos := cocoa.NSWindow{ID: w.platform.object}.MouseLocationOutsideOfEventStream()
...
func (w *Window) platformWindowFocused() bool {
	pool := cocoa.NSAutoreleasePool_new()
	defer pool.Release()
	return cocoa.NSWindow{w.platform.object}.IsKeyWindow()
}
...
func (c *Cursor) platformCreateStandardCursor(shape StandardCursor) error {
	...
	if c.platform.object == 0 {
		...
		case ArrowCursor:
			...
			panic("TODO")
		case IBeamCursor:
			c.platform.object = cocoa.NSCursor_IBeamCursor().ID
		case CrosshairCursor:
			c.platform.object = cocoa.NSCursor_crosshairCursor().ID
		case HandCursor:
			c.platform.object = cocoa.NSCursor_pointingHandCursor().ID
		case HResizeCursor:
	...
	cocoa.NSObject_retain(c.platform.object)
	return nil
}
...
func translateKey(key uint16) Key {
	if key >= uint16(len(_glfw.platformWindow.keycodes)) {
		return KeyUnknown
	}
	return _glfw.platformWindow.keycodes[key]
}
