...
// normalizeSwizzlingForHLSL converts a swizzling to one HLSL accepts.
// HLSL supports only the xyzw and rgba component naming sets, so stpq is
// rewritten to the equivalent xyzw swizzling.
// https://learn.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-hlsl-per-component-math#the-vector-type
func normalizeSwizzlingForHLSL(s string) string {
	const (
		stpq = "stpq"
		xyzw = "xyzw"
	)
...
func (c *compileContext) block(p *shaderir.Program, topBlock, block *shaderir.Block, level int) []string {
	...
	expr = func(e *shaderir.Expr) string {
		...
		case shaderir.SwizzlingExpr:
			...
			return normalizeSwizzlingForHLSL(e.Swizzling)
