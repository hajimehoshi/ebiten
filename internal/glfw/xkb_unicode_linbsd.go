...
func _glfwKeySym2Unicode(keysym C.uint) C.uint32_t {
	...
	idx, found := slices.BinarySearchFunc(keysymtab, codepair{keysym: uint32(keysym)}, func(a, b codepair) int {
		return int(a.keysym) - int(b.keysym)
	})
	if !found {
		return C.GLFW_INVALID_CODEPOINT
	}
	return C.uint32_t(keysymtab[idx].ucs)
}
