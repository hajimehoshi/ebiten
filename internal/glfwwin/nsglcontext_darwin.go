...
func createContextNSGL(window *Window, ctxconfig *ctxconfig, fbconfig *fbconfig) error {
	...
	var attribs = make([]NSOpenGLPixelFormatAttribute, 0, 40)
	...
	window.context.state.pixelFormat = NSOpenGLPixelFormat_alloc().initWithAttributes(attribs).ID
	if window.context.state.pixelFormat == 0 {
		return fmt.Errorf("NSGL: Failed to find a suitable pixel format")
	}
	var share NSOpenGLContext
	if ctxconfig.share != nil {
		share = NSOpenGLContext{ctxconfig.share.context.state.object}
	}

	window.context.state.object = NSOpenGLContext_alloc().initWithFormat_shareContext(NSOpenGLPixelFormat{window.context.state.pixelFormat}, share).ID
	if window.context.state.object == 0 {
	...
	cocoa.NSView{ID: window.state.view}.SetWantsBestResolutionOpenGLSurface(window.state.retina)
	cocoa.NSWindow{ID: window.context.state.object}.SetView(window.state.view)
...
func swapBuffersNSGL(w *Window) error {
	...
	if w.state.occluded {
		interval := 0
		NSOpenGLContext{w.context.state.object}.setValues_forParameter(&interval, NSOpenGLContextParameterSwapInterval)
	...
	NSOpenGLContext{w.context.state.object}.flushBuffer()
	return nil
}
...
func getProcAddressNSGL(s string) uintptr {
	...
	symbol := cf.CFBundleGetFunctionPointerForName(_glfw.context.framework, symbolName)
...
func makeContextCurrentNSGL(window *Window) error {
	...
	defer pool.Release()
	if window != nil {
		NSOpenGLContext{window.context.state.object}.makeCurrentContext()
	} else {
...
func swapIntervalNSGL(interval int) error {
	...
	if window != nil {
		NSOpenGLContext{window.context.state.object}.setValues_forParameter(&interval, NSOpenGLContextParameterSwapInterval)
	}
	return nil
}
