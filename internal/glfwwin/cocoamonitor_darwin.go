...
func (m *Monitor) platformGetMonitorContentScale() (xscale, yscale float32, err error) {
	...
	//    }
	points := m.state.screen.Frame()
	pixels := m.state.screen.ConvertRectToBacking(points)
...
func (m *Monitor) platformGetVideoMode() *VidMode {
	...
	//    @autoreleasepool {
	//
	native := _CGDisplayCopyDisplayMode(m.state.displayID)
	mode := vidmodeFromCGDisplayMode(native, m.state.fallbackRefreshRate)
...
func pollMonitorsCocoa() error {
	...
	for i := uint32(0); i < displayCount; i++ {
		...
		for j = 0; j < len(disconnected); j++ {
			...
			//        {
			if disconnected[j] != nil && disconnected[j].state.unitNumber == unitNumber {
				disconnected[j].state.screen = screen
				disconnected[j] = nil
				break
			}
		...
		monitor.state.displayID = displays[i]
		monitor.state.unitNumber = unitNumber
		monitor.state.screen = screen
